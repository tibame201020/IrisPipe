  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <ActionButton tone="primary" onClick={() => setEditingId('new')}>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-2 py-10 text-base-content/40">
          ...
          <ActionButton size="xs" tone="ghost" onClick={() => setEditingId('new')}>Add your first connection</ActionButton>
        </div>
      ) : (
        <div className="iris-list-panel flex flex-col">
          {connections.map((conn) => (
            <div key={conn.id} className="iris-list-row flex items-center gap-3 bg-base-100 px-4 py-3 transition-colors">
              ...
              <ActionButton size="xs" tone="icon" square onClick={() => setEditingId(conn.id)}><Pencil size={12} /></ActionButton>
              <ActionButton size="xs" tone="icon" square className="text-error/60" onClick={() => setDeleteTarget(conn)}><Trash2 size={12} /></ActionButton>
            </div>
          ))}
        </div>
      )}
      ...
      {deleteTarget ? (
        <DialogShell
          ...
          footer={(
            <>
              <ActionButton tone="ghost" onClick={() => setDeleteTarget(null)}>Cancel</ActionButton>
              <ActionButton tone="danger" disabled={deleteSubmitting} onClick={() => void handleDelete()}>
...
function ConnectionFormModal({
  ...
  return (
    <DialogShell
      open
      title={id === 'new' ? 'New Connection' : 'Edit Connection'}
      maxWidthClassName="max-w-lg"
      onClose={onClose}
      footer={(
        <>
          <ActionButton tone="ghost" onClick={onClose}>Cancel</ActionButton>
          <ActionButton tone="primary" disabled={saving || !canSave} onClick={handleSave}>
              ...
              <div className="grid gap-5 xl:grid-cols-[minmax(280px,0.7fr)_minmax(0,1fr)]">
                <section className="iris-list-panel flex flex-col gap-4 px-5 py-5">
                  ...
                  <SummaryTile
                    ...
                    value="Approved Only"
                    detail="Only approved daisyUI presets are exposed. Theme switching must preserve text, border, inset, code, and status readability."
                    tone="info"
                    icon={<X size={14} className="text-error" />}
                  />

                  <SurfaceBox variant="glassSoft" className="px-4 py-4">
                ...
                <section className="iris-list-panel overflow-hidden p-0">
                  <PanelHeader
                    kicker="Approved Themes"
                    detail="Choose between the supported console themes."
                    aside={<span className="iris-mono-meta uppercase tracking-[0.18em]">daisyUI presets</span>}
                  />
