...
export default function (data) {
    ...
    const stoppedDetail = getPipelineRunDetailOrFail(summary.id, 'stopped job pipeline detail query');
    const stoppedDestACount = queryScalarOrFail(
        'SELECT COUNT(*) AS CNT FROM test_stop_job_dest_a',
        'CNT',
        'stopped job dest a count',
    );
    const stoppedDestBCount = queryScalarOrFail(
        'SELECT COUNT(*) AS CNT FROM test_stop_job_dest_b',
        'CNT',
        'stopped job dest b count',
    );

    check(startedSummary, {
        'Stop job pipeline reached STARTED before stop': (item) => item.status === 'STARTED',
    });
    check(stopRequestedSummary, {
        'Stop job request returns the same pipeline run id': (item) => item.id === summary.id,
    });
    check(stoppedSummary, {
        'Stop job pipeline eventually reaches STOPPED': (item) => item.status === 'STOPPED',
    });
    check(stoppedDetail, {
        'Stopped job detail marks first job STOPPED': (item) =>
            Array.isArray(item.jobs) && item.jobs.length === 2 && item.jobs[0].status === 'STOPPED',
        'Stopped job detail marks downstream job NOT_RUN': (item) =>
            Array.isArray(item.jobs) && item.jobs.length === 2 && item.jobs[1].status === 'NOT_RUN',
    });
    check(stoppedDestACount, {
        'Stopped atomic JOB rolls back the in-flight destination changes': (count) => count === 0,
    });
    check(stoppedDestBCount, {
        'Stopped atomic JOB does not start downstream job': (count) => count === 0,
    });
    ...
    const completedDetail = getPipelineRunDetailOrFail(summary.id, 'resumed job pipeline detail query');
    const completedDestACount = queryScalarOrFail(
        'SELECT COUNT(*) AS CNT FROM test_stop_job_dest_a',
        'CNT',
        'completed job dest a count',
    );
    const completedDestBCount = queryScalarOrFail(
        'SELECT COUNT(*) AS CNT FROM test_stop_job_dest_b',
        'CNT',
        'completed job dest b count',
    );
    const completedExecutionCount = queryScalarOrFail(
        ...
        'CNT',
        'completed job execution count',
    );

    check(resumedSummary, {
        'JOB resume after stop keeps the same pipeline run id': (item) => item.id === summary.id,
    });
    check(completedSummary, {
        'JOB resume after stop eventually completes': (item) => item.status === 'COMPLETED',
    });
    check(completedDetail, {
        'Resumed JOB detail marks first job COMPLETED': (item) =>
            Array.isArray(item.jobs) && item.jobs.length === 2 && item.jobs[0].status === 'COMPLETED',
        'Resumed JOB detail marks downstream job COMPLETED': (item) =>
            Array.isArray(item.jobs) && item.jobs.length === 2 && item.jobs[1].status === 'COMPLETED',
    });
    check(completedDestACount, {
        'Resumed atomic JOB replays the full first destination load': (count) => count === totalRows,
    });
    check(completedDestBCount, {
        'Resumed atomic JOB runs downstream job after recovery': (count) => count === downstreamRows,
    });
    check(completedExecutionCount, {
        'JOB resume after stop stores two execution attempts': (count) => count === 2,
    });

    deletePipelineRunOrFail(summary.id, 'stopped job pipeline run delete');
}
